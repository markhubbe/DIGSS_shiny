#'Survey Simulations - SHINY VERSION
#'
#'Master function that runs survey simulations.
#'This is adjusted code from the DIGSS package to take full advantage of Shiny interface
#'
#'@details `surveySimShiny` is the main function that will conduct the simulations
#'of survey grids based on the Survey Parameters created by user.
#'@param SurveyParameters List of class `SurveySim` generated by `SurveyParameters()`
...
surveySimShiny<-function(SurveyParameters, artifact.analysis=TRUE, plot=TRUE, plot.artifacts=FALSE,areaprecision=1000, grid_ratio=NA){
  ...
  withProgress(message=paste0("Simulation: ",0,"/",simulations),min=0,max=simulations,value=0,{
    for (a in 1:simulations){
      ...
      sitemap<-fieldMapShiny(Area,site.density,site.area,overlap,plot=FALSE,areaprecision=areaprecision)
