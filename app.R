...
ui <- fluidPage(
    ...
    #enable shinyJs to control fields enabled or not
    shinyjs::useShinyjs(),
        ...
        mainPanel(
            ...
            tags$table(border=0, style = "width:100%",
                       ...
                       tags$tbody(align = "left",
                                  tags$tr(
                                      tags$td(style = ("width: 50%"), plotOutput("sitemap",width="600px",height="600px")),
...
server <- function(input, output, session) {
    ...
    observeEvent(input$run,{
        ...
        output$sitemap<-renderPlot(replayPlot(survey_results$plot))
        ...
        output$summary_plot<-renderPlot(PlotSurveySumm(list(survey_results),input$summ_plot,labels = results_labels[sims_done]))
    ...
    observeEvent(input$summ_plot,{
        if(results_exist==TRUE){
            output$summary_plot<-renderPlot(PlotSurveySumm(results[res_to_plot],input$summ_plot,labels = results_labels[res_to_plot]))
        }
    })
    ...
    observeEvent(input$loop_run,{
        ...
        #erase some results, to remove empty spaces
        output$welcome<-NULL
    ...
    RunLoop<-function(SurveyParameters,loop_var,loop_pars,grid_ratio=NULL){
        ...
        p<-ggplot(data=tmp,aes(x=`Loop Value`,y=Mean,group=1), color=col_palette[1])+
            geom_line(size=1)+
            geom_point(size=4)+
    ...
    observeEvent(input$loop_plot,{
        if(is.null(loop_results)==FALSE){
            ...
            if(input$loop_plot=="SitesFound"){
                ...
                p<-ggplot(data=tmp,aes(x=`Loop Value`,y=Mean,group=1), color=col_palette[2])+
                    geom_line(size=1)+
                    geom_point(size=4)+
                    geom_errorbar(aes(ymin= Mean-`St Dev`, ymax= Mean+`St Dev`), width=.1)+
                ...
                p<-ggplot(data=tmp,aes(x=`Loop Value`,y=Mean,group=1), color=col_palette[3])+
                    geom_line(size=1)+
                    geom_point(size=4)+
                    geom_errorbar(aes(ymin= Mean-`St Dev`, ymax= Mean+`St Dev`), width=.1)+
                ...
                p<-ggplot(data=tmp,aes(x=`Loop Value`,y=Mean,group=1), color=col_palette[4])+
                    geom_line(size=1)+
                    geom_point(size=4)+
                    geom_errorbar(aes(ymin= Mean-`St Dev`, ymax= Mean+`St Dev`), width=.1)+
                ...
                p<-ggplot(data=tmp,aes(x=`Loop Value`,y=N,group=1), color=col_palette[5])+
                    geom_line(size=1)+
                    geom_point(size=4)+
                    xlab(x_lab)+
                    theme_minimal()
    ...
    load_parameters<-function(){
        ...
        #SelPar<-data.frame("Value"=c(SurveyParameters$col.width,SurveyParameters$grid.type,SurveyParameters$simulations,SurveyParameters$area[1],SurveyParameters$area[2],
        #                            SurveyParameters$site.density,SurveyParameters$site.area, SurveyParameters$overlap,SurveyParameters$obj.distribution,SurveyParameters$survey.radius))
